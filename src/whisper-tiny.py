...
def play_wav(filepath: str):
    """Play back a WAV/AIFF/FLAC file using the default output device."""
    data, samplerate = sf.read(filepath, dtype="float32", always_2d=True)
    print(f"🔊 Playing: {os.path.abspath(filepath)}  |  {samplerate} Hz")
    sd.play(data, samplerate)
    sd.wait()  # wait until playback finishes
    print("⏹️ Playback done.")
